...
# Linear predictor
predict(mod_ref_log_c, newdata = DisturbedDreams[ind, ], type = "linear.predict")
```
...
mod3 <- glmcat(
  ...
  data = DisturbedDreams, cdf = student_glmcat(df = 0.5)
)
summary(mod3)
logLik(mod3)
```
...
cum_log_c <- glmcat(
  formula = Level ~ Age,
  cdf = student_glmcat(df = 0.8),
  ratio = "cumulative",
  data = DisturbedDreams,
  control = glmcat_control(beta_init = coef(cum_log_co))
)
logLik(cum_log_c)
summary(cum_log_c)
```
